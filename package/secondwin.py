...
class Secondwindow:
    def __init__(self,mainwindow):
        ...
        self.tableop = LabelFrame(self.frame,text = "Table Operations.",bg="#2a2d2e",fg="white")
        ...
        self.view = CTkButton(self.tableop,text="View Table",command=self.showtable,height=2,width=14,borderwidth=4,bg_color="#565b5e",fg_color="#565b5e").grid(row = 1,column=3,padx=30,pady=10)
        self.tablelist=CTkButton(self.tableop,text= "Table List",command=self.showtablelist,height=2,width=14,borderwidth=4,bg_color="#565b5e",fg_color="#565b5e").grid(row = 3,column=3,padx=30,pady=10)
        self.save = CTkButton(self.tableop,text = 'Save Table',height=2,width=14,borderwidth=4,command = lambda:threading.Thread(target=self.convert_table_to_xls).start(),bg_color="#565b5e",fg_color="#565b5e").grid(row=5,column=3,padx=30,pady=10)
        self.saveall = CTkButton(self.tableop,text = 'Save All Table',height=2,width=14,borderwidth=4,command = lambda:threading.Thread(target = self.savealltable,args=(self.root,)).start(),bg_color="#565b5e",fg_color="#565b5e").grid(row=1,column=5,padx=30,pady=10)
        self.path = ''
        self.breplace = CTkButton(self.tableop,text = "Replace Table",command= lambda:threading.Thread(target=self.replace).start(),bg_color="#565b5e",fg_color="#565b5e")
        ...
        self.b = CTkButton(self.tableop,text = "Browse",command= self.openex,bg_color="#565b5e",fg_color="#565b5e")
        ...
        self.inputdata = LabelFrame(self.frame,text = "Input Data",bg="#2a2d2e",fg="white")
        self.inputdata.grid(row=0,column=2,pady=10,padx=10)

        self.input_ledger = CTkButton(self.inputdata,text="Input Data In ledger",command=lambda:threading.Thread(target = self.insert_ledger).start(),bg_color="#565b5e",fg_color="#565b5e")
        self.input_ledger.grid(row=0,column=0,pady=20,padx=20)

        self.input_stockitem = CTkButton(self.inputdata,text="Input Data In stockitem",command=self.insert_stockitem,bg_color="#565b5e",fg_color="#565b5e")
        self.input_stockitem.grid(row=1,column=0,pady=20,padx=20)

        self.sales02 = CTkButton(self.inputdata,text="Input Data In 02sales",command=self.insert_02sales,bg_color="#565b5e",fg_color="#565b5e")
        self.sales02.grid(row=2,column=0,pady=20,padx=20)


        self.purchase02 = CTkButton(self.inputdata,text="Input Data In 02purchase",command=self.insert_02purchase,bg_color="#565b5e",fg_color="#565b5e")
        self.purchase02.grid(row=3,column=0,pady=20,padx=20)

        
        self.sale03 = CTkButton(self.inputdata,text="Export Data From ldgr",command=self.export_ldgr,bg_color="#565b5e",fg_color="#565b5e")
        self.sale03.grid(row=4,column=0,pady=20,padx=20)

        self.back_img=  ImageTk.PhotoImage(Image.open("src/img/back.png").resize((40,40),Image.ANTIALIAS))
        self.back_CTkButton = CTkButton(self.frame,text = "" ,height =30,width =30,image=self.back_img,command=self.back)
        self.back_CTkButton.configure(bg_color="#2a2d2e",fg_color = "#2a2d2e")
        self.back_CTkButton.grid(row=0,column=0,padx=10,pady=20,sticky= "NE")


        self.xmlcon = LabelFrame(self.frame,text = "XML Conversion",bg="#2a2d2e",fg="white")
    ...
    def settablemenue(self):
        ...
        if len(self.table_names)>0:
            ...
            self.clicked.set('')
        self.dbox = CTkComboBox(master = self.tableop,variable = self.clicked,values = self.table_names)
    ...
    def savealltable(self,root):
        ...
        try:
            ...
            for tname in self.table_names:
                l.configure(text=f"Saving {tname} ({i}/{len(self.table_names)})")
    ...
    def showtable(self):
        ...
        try:
            ...
            scrollv  = CTkScrollbar(frame,orientation=VERTICAL,command=tree.yview)
            scrollv.pack(side =RIGHT, fill = Y)
            scrollh  = CTkScrollbar(frame,orientation=HORIZONTAL,command=tree.xview)
            scrollh.pack(side =BOTTOM, fill = X)

            tree.pack(side='left',fill= BOTH)
            tree.configure(yscrollcommand=scrollv.set,xscrollcommand=scrollh.set)
            ...
            for res in result:
                if count%2==0:
                    tree.insert(parent='', index='end',text="", values=res, tags=('evenrow',))
                else:
                    tree.insert(parent='', index='end',text="", values=res, tags=('oddrow',))
                count+=1
    ...
    def openex(self):
        filename = filedialog.askopenfilename(title="Select File",filetypes=[("Excel files","*.xlsx"),("csv files","*.csv"),("dbf files","*.dbf")])
        ...
        try:
            ...
            if len(self.path)>0:
                ...
                if tname not in self.table_names:
                    messagebox.showerror("Error","File name and table name must be same.")
                else:
    ...
    def convert_table_to_xls(self):
        self.f = filedialog.asksaveasfilename(filetypes=[("Excel files", "*.xlsx")],defaultextension=".xlsx")
        ...
        try:
            ...
            pb = ttk.Progressbar(savewin, orient= HORIZONTAL, mode= 'indeterminate')
            pb.pack(anchor="c",padx=20,pady=20)
    ...
    def showtablelist(self):
        ...
        for i in range(len(self.table_names)):
            table.insert(parent='',index='end',iid = i,text="",values=(i+1,self.table_names[i]))
    ...
    def insert_ledger(self):
        ...
        if 'master'  in t_names:
            ...
            try:
                ...
                cur.execute("UPDATE sqlite_sequence SET seq = 1 WHERE name = 'ledger';")
                cur.execute(query['input_ledger'])
                if 'master' in t_names and 'ldgr' in t_names:
                    cur.execute("UPDATE ldgr SET LEDGER_NAME = (SELECT name from master where master.code = ldgr.code);")
                con.commit()
                con.close()
    ...
    def insert_stockitem(self):
        ...
        if 'product' in t_names:
            ...
            stock_names = self.cursor.execute("SELECT B_DESC FROM PRODUCT WHERE TAX != 14.5").fetchall()
            si = input_stockitem(self.tableop,self.conn,self.cursor,stock_names)
            si.input_date()
        else:
    ...
    def Enter(self):    
        ...
        l = CTkLabel(master = self.selectwin, text = "Select Exporting Style :").grid(row=0,column=0,padx=20,pady=10)
        l1  = CTkLabel(master = self.selectwin, text = "AccMultipleDaybookStyle").grid(row=0,column=1,padx=20,pady=10)
        l2  = CTkLabel(master = self.selectwin, text = "AccountingSingleVch").grid(row=0,column=2,padx=20,pady=10)
        
        self.export = CTkButton(master=self.selectwin,text="Export",command=self.ldgr_ex)
        self.export.grid(row=7,column=3,pady=10,padx=10)
        self.export.configure(state='disabled')
        

        sales = CTkLabel(master= self.selectwin,text="Sales").grid(row=1,column=0,padx=20,pady=10)
        sm = CTkRadioButton(master=self.selectwin, text="",variable= self.s, value=1,command = self.change_state).grid(row=1,column=1,padx=20,pady=10)
        ss = CTkRadioButton(master=self.selectwin, text="",variable= self.s, value=2,command = self.change_state).grid(row=1,column=2,padx=20,pady=10)

        pur = CTkLabel(master= self.selectwin,text="Purchase").grid(row=2,column=0,padx=20,pady=10)
        pm = CTkRadioButton(master=self.selectwin, text="",variable= self.p, value=1,command = self.change_state).grid(row=2,column=1,padx=20,pady=10)
        ps = CTkRadioButton(master=self.selectwin, text="",variable= self.p, value=2,command = self.change_state).grid(row=2,column=2,padx=20,pady=10)

        cash_l = CTkLabel(master= self.selectwin,text="Cash Deposit").grid(row=3,column=0,padx=20,pady=10)
        cam = CTkRadioButton(master=self.selectwin, text="",variable= self.cash, value=1,command = self.change_state).grid(row=3,column=1,padx=20,pady=10)
        cas = CTkRadioButton(master=self.selectwin, text="",variable= self.cash, value=2,command = self.change_state).grid(row=3,column=2,padx=20,pady=10)
        cas = CTkRadioButton(master=self.selectwin, text="",variable= self.cash, value=2,command = self.change_state).grid(row=3,column=2,padx=20,pady=10)

        check_l = CTkLabel(master= self.selectwin,text="Check Deposit").grid(row=4,column=0,padx=20,pady=10)
        chm = CTkRadioButton(master=self.selectwin, text="",variable= self.check, value=1,command = self.change_state).grid(row=4,column=1,padx=20,pady=10)
        chs = CTkRadioButton(master=self.selectwin, text="",variable= self.check, value=2,command = self.change_state).grid(row=4,column=2,padx=20,pady=10)

        rec = CTkLabel(master= self.selectwin,text="Receipt").grid(row=5,column=0,padx=20,pady=10)
        rm = CTkRadioButton(master=self.selectwin, text="",variable= self.r, value=1,command = self.change_state).grid(row=5,column=1,padx=20,pady=10)
        rs = CTkRadioButton(master=self.selectwin, text="",variable= self.r, value=2,command = self.change_state).grid(row=5,column=2,padx=20,pady=10)

        pay_l = CTkLabel(master= self.selectwin,text="Payment").grid(row=6,column=0,padx=20,pady=10)
        pam = CTkRadioButton(master=self.selectwin, text="",variable= self.pay, value=1,command = self.change_state).grid(row=6,column=1,padx=20,pady=10)
        pas = CTkRadioButton(master=self.selectwin, text="",variable= self.pay, value=2,command = self.change_state).grid(row=6,column=2,padx=20,pady=10)
    ...
    def ldgr_ex(self):
        ...
        if self.r.get() == 1:
            self.cursor.execute(query['insert_receipt_AccMultipleDaybookStyle'])
        else:
        ...
        self.cursor.execute("CREATE  VIEW cashrec AS SELECT * from AccMultipleDaybookStyle WHERE source_vouchertypename = 'Cash Receipt';")
        self.cursor.execute("CREATE  VIEW discount AS SELECT * from AccMultipleDaybookStyle WHERE source_vouchertypename = 'Discount';")
        ...
        if self.pay.get() == 1:
            self.cursor.execute(query['insert_payment_AccMultipleDaybookStyle'])
        else:
            self.cursor.execute(query['insert_payment_AccountingSingleVch'])

        if self.cash.get() == 1:
            self.cursor.execute(query['insert_cashdeposit_AccMultipleDaybookStyle'])
        else:
            self.cursor.execute(query['insert_scashdeposit_AccountingSingleVch'])
